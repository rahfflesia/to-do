...
class DOM {
    ...
    enableProject(allProjects, projectTitleGUI){
        ...
        for(let i = 0; i < allProjects.length; i++){
            allProjects[i].addEventListener('click', () => {
                const retrievedProject = JSON.parse(localStorage.getItem(allProjects[i].textContent));
    ...
    appendTasksToProject(projectName, containerToAppend){
        ...
        for(let i = 0; i < tasks.length; i++){
            ...
            this.enableDeleteButton(deleteIcon, taskDiv, project, tasks, taskTitle, projectName); 

            this.enableCheckbox(input, taskDiv, project, tasks, taskTitle, projectName);

            containerToAppend.appendChild(taskDiv);

            this.showProjectDetails(detailsIcon, taskTitle, description, date, priority);

            const data = Task.getProjectDetails(taskTitle.textContent, description.textContent, date.textContent, priority.textContent);
            this.showEditDialog(editIcon, data, taskTitle, description, date, priority);
        }
    }
    ...
    enableDeleteButton(button, parentDiv, project, tasks, taskTitle, projectName){
        button.addEventListener('click', () => {
        ...
        editIcon.addEventListener('click', () => {
            ...
            edit.addEventListener('click', () => {
                ...
                for(let i = 0; i < allTasks.length; i++){
                    ...
                    const newTask = new Task(info[1].textContent, info[4].textContent, info[3].textContent, info[2].textContent);
